...
func activate_ability(_target: Node2D = null , _activator: Node2D = null, _result: DamageResult = null) -> void:
	if _target and _target is Enemy and _activator is Arrow and _activator.perfect_shot:
		...
		poison_debuff.poison_damage = PlayerManager.player.stats.poison_damage
		var poison_duration = PlayerManager.player.stats.poison_duration
		var poison_ticks = PlayerManager.player.stats.poison_ticks
